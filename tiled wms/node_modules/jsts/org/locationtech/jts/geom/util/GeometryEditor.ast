{
  "sourceType" : "module",
  "body" : [ {
    "specifiers" : [ {
      "local" : {
        "name" : "LineString",
        "type" : "Identifier"
      },
      "type" : "ImportDefaultSpecifier"
    } ],
    "source" : {
      "value" : "'../LineString'",
      "raw" : "'../LineString'",
      "type" : "Literal"
    },
    "type" : "ImportDeclaration"
  }, {
    "specifiers" : [ {
      "local" : {
        "name" : "Geometry",
        "type" : "Identifier"
      },
      "type" : "ImportDefaultSpecifier"
    } ],
    "source" : {
      "value" : "'../Geometry'",
      "raw" : "'../Geometry'",
      "type" : "Literal"
    },
    "type" : "ImportDeclaration"
  }, {
    "specifiers" : [ {
      "local" : {
        "name" : "Point",
        "type" : "Identifier"
      },
      "type" : "ImportDefaultSpecifier"
    } ],
    "source" : {
      "value" : "'../Point'",
      "raw" : "'../Point'",
      "type" : "Literal"
    },
    "type" : "ImportDeclaration"
  }, {
    "specifiers" : [ {
      "local" : {
        "name" : "Polygon",
        "type" : "Identifier"
      },
      "type" : "ImportDefaultSpecifier"
    } ],
    "source" : {
      "value" : "'../Polygon'",
      "raw" : "'../Polygon'",
      "type" : "Literal"
    },
    "type" : "ImportDeclaration"
  }, {
    "specifiers" : [ {
      "local" : {
        "name" : "LinearRing",
        "type" : "Identifier"
      },
      "type" : "ImportDefaultSpecifier"
    } ],
    "source" : {
      "value" : "'../LinearRing'",
      "raw" : "'../LinearRing'",
      "type" : "Literal"
    },
    "type" : "ImportDeclaration"
  }, {
    "specifiers" : [ {
      "local" : {
        "name" : "GeometryCollection",
        "type" : "Identifier"
      },
      "type" : "ImportDefaultSpecifier"
    } ],
    "source" : {
      "value" : "'../GeometryCollection'",
      "raw" : "'../GeometryCollection'",
      "type" : "Literal"
    },
    "type" : "ImportDeclaration"
  }, {
    "specifiers" : [ {
      "local" : {
        "name" : "ArrayList",
        "type" : "Identifier"
      },
      "type" : "ImportDefaultSpecifier"
    } ],
    "source" : {
      "value" : "'../../../../../java/util/ArrayList'",
      "raw" : "'../../../../../java/util/ArrayList'",
      "type" : "Literal"
    },
    "type" : "ImportDeclaration"
  }, {
    "specifiers" : [ {
      "local" : {
        "name" : "Assert",
        "type" : "Identifier"
      },
      "type" : "ImportDefaultSpecifier"
    } ],
    "source" : {
      "value" : "'../../util/Assert'",
      "raw" : "'../../util/Assert'",
      "type" : "Literal"
    },
    "type" : "ImportDeclaration"
  }, {
    "declaration" : {
      "id" : {
        "name" : "GeometryEditor",
        "type" : "Identifier"
      },
      "body" : {
        "body" : [ {
          "key" : {
            "name" : "constructor",
            "type" : "Identifier"
          },
          "value" : {
            "params" : null,
            "body" : {
              "body" : [ {
                "expression" : {
                  "callee" : {
                    "object" : {
                      "name" : "GeometryEditor",
                      "type" : "Identifier"
                    },
                    "property" : {
                      "object" : {
                        "name" : "constructor_",
                        "type" : "Identifier"
                      },
                      "property" : {
                        "name" : "apply",
                        "type" : "Identifier"
                      },
                      "computed" : false,
                      "type" : "MemberExpression"
                    },
                    "computed" : false,
                    "type" : "MemberExpression"
                  },
                  "arguments" : [ {
                    "type" : "ThisExpression"
                  }, {
                    "name" : "arguments",
                    "type" : "Identifier"
                  } ],
                  "type" : "CallExpression"
                },
                "type" : "ExpressionStatement"
              } ],
              "type" : "BlockStatement"
            },
            "generator" : false,
            "type" : "FunctionExpression"
          },
          "kind" : "constructor",
          "computed" : false,
          "static" : false,
          "type" : "MethodDefinition"
        }, {
          "key" : {
            "name" : "constructor_",
            "type" : "Identifier"
          },
          "value" : {
            "params" : [ ],
            "body" : {
              "body" : [ {
                "expression" : {
                  "operator" : "=",
                  "left" : {
                    "object" : {
                      "type" : "ThisExpression"
                    },
                    "property" : {
                      "name" : "_factory",
                      "type" : "Identifier"
                    },
                    "computed" : false,
                    "type" : "MemberExpression"
                  },
                  "right" : {
                    "value" : null,
                    "raw" : "null",
                    "type" : "Literal"
                  },
                  "type" : "AssignmentExpression"
                },
                "type" : "ExpressionStatement"
              }, {
                "expression" : {
                  "operator" : "=",
                  "left" : {
                    "object" : {
                      "type" : "ThisExpression"
                    },
                    "property" : {
                      "name" : "_isUserDataCopied",
                      "type" : "Identifier"
                    },
                    "computed" : false,
                    "type" : "MemberExpression"
                  },
                  "right" : {
                    "value" : false,
                    "raw" : "false",
                    "type" : "Literal"
                  },
                  "type" : "AssignmentExpression"
                },
                "type" : "ExpressionStatement"
              }, {
                "test" : {
                  "operator" : "===",
                  "left" : {
                    "name" : "arguments.length",
                    "type" : "Identifier"
                  },
                  "right" : {
                    "value" : 0,
                    "raw" : "0",
                    "type" : "Literal"
                  },
                  "type" : "BinaryExpression"
                },
                "consequent" : {
                  "body" : [ ],
                  "type" : "BlockStatement"
                },
                "alternate" : {
                  "test" : {
                    "operator" : "===",
                    "left" : {
                      "name" : "arguments.length",
                      "type" : "Identifier"
                    },
                    "right" : {
                      "value" : 1,
                      "raw" : "1",
                      "type" : "Literal"
                    },
                    "type" : "BinaryExpression"
                  },
                  "consequent" : {
                    "body" : [ {
                      "declarations" : [ {
                        "id" : {
                          "name" : "factory",
                          "type" : "Identifier"
                        },
                        "init" : {
                          "object" : {
                            "name" : "arguments",
                            "type" : "Identifier"
                          },
                          "property" : {
                            "value" : 0,
                            "raw" : "0",
                            "type" : "Literal"
                          },
                          "computed" : true,
                          "type" : "MemberExpression"
                        },
                        "type" : "VariableDeclarator"
                      } ],
                      "kind" : "let",
                      "type" : "VariableDeclaration"
                    }, {
                      "expression" : {
                        "operator" : "=",
                        "left" : {
                          "object" : {
                            "type" : "ThisExpression"
                          },
                          "property" : {
                            "name" : "_factory",
                            "type" : "Identifier"
                          },
                          "computed" : false,
                          "type" : "MemberExpression"
                        },
                        "right" : {
                          "name" : "factory",
                          "type" : "Identifier"
                        },
                        "type" : "AssignmentExpression"
                      },
                      "type" : "ExpressionStatement"
                    } ],
                    "type" : "BlockStatement"
                  },
                  "alternate" : null,
                  "type" : "IfStatement"
                },
                "type" : "IfStatement"
              } ],
              "type" : "BlockStatement"
            },
            "generator" : false,
            "type" : "FunctionExpression"
          },
          "kind" : "method",
          "computed" : false,
          "static" : true,
          "type" : "MethodDefinition"
        }, {
          "key" : {
            "name" : "setCopyUserData",
            "type" : "Identifier"
          },
          "value" : {
            "params" : [ {
              "name" : "isUserDataCopied",
              "type" : "Identifier"
            } ],
            "body" : {
              "body" : [ {
                "expression" : {
                  "operator" : "=",
                  "left" : {
                    "object" : {
                      "type" : "ThisExpression"
                    },
                    "property" : {
                      "name" : "_isUserDataCopied",
                      "type" : "Identifier"
                    },
                    "computed" : false,
                    "type" : "MemberExpression"
                  },
                  "right" : {
                    "name" : "isUserDataCopied",
                    "type" : "Identifier"
                  },
                  "type" : "AssignmentExpression"
                },
                "type" : "ExpressionStatement"
              } ],
              "type" : "BlockStatement"
            },
            "generator" : false,
            "type" : "FunctionExpression"
          },
          "kind" : "method",
          "computed" : false,
          "static" : false,
          "type" : "MethodDefinition"
        }, {
          "key" : {
            "name" : "edit",
            "type" : "Identifier"
          },
          "value" : {
            "params" : [ {
              "name" : "geometry",
              "type" : "Identifier"
            }, {
              "name" : "operation",
              "type" : "Identifier"
            } ],
            "body" : {
              "body" : [ {
                "test" : {
                  "operator" : "===",
                  "left" : {
                    "name" : "geometry",
                    "type" : "Identifier"
                  },
                  "right" : {
                    "value" : null,
                    "raw" : "null",
                    "type" : "Literal"
                  },
                  "type" : "BinaryExpression"
                },
                "consequent" : {
                  "argument" : {
                    "value" : null,
                    "raw" : "null",
                    "type" : "Literal"
                  },
                  "type" : "ReturnStatement"
                },
                "alternate" : null,
                "type" : "IfStatement"
              }, {
                "declarations" : [ {
                  "id" : {
                    "name" : "result",
                    "type" : "Identifier"
                  },
                  "init" : {
                    "callee" : {
                      "object" : {
                        "type" : "ThisExpression"
                      },
                      "property" : {
                        "name" : "editInternal",
                        "type" : "Identifier"
                      },
                      "computed" : false,
                      "type" : "MemberExpression"
                    },
                    "arguments" : [ {
                      "name" : "geometry",
                      "type" : "Identifier"
                    }, {
                      "name" : "operation",
                      "type" : "Identifier"
                    } ],
                    "type" : "CallExpression"
                  },
                  "type" : "VariableDeclarator"
                } ],
                "kind" : "let",
                "type" : "VariableDeclaration"
              }, {
                "test" : {
                  "object" : {
                    "type" : "ThisExpression"
                  },
                  "property" : {
                    "name" : "_isUserDataCopied",
                    "type" : "Identifier"
                  },
                  "computed" : false,
                  "type" : "MemberExpression"
                },
                "consequent" : {
                  "body" : [ {
                    "expression" : {
                      "callee" : {
                        "object" : {
                          "name" : "result",
                          "type" : "Identifier"
                        },
                        "property" : {
                          "name" : "setUserData",
                          "type" : "Identifier"
                        },
                        "computed" : false,
                        "type" : "MemberExpression"
                      },
                      "arguments" : [ {
                        "callee" : {
                          "object" : {
                            "name" : "geometry",
                            "type" : "Identifier"
                          },
                          "property" : {
                            "name" : "getUserData",
                            "type" : "Identifier"
                          },
                          "computed" : false,
                          "type" : "MemberExpression"
                        },
                        "arguments" : [ ],
                        "type" : "CallExpression"
                      } ],
                      "type" : "CallExpression"
                    },
                    "type" : "ExpressionStatement"
                  } ],
                  "type" : "BlockStatement"
                },
                "alternate" : null,
                "type" : "IfStatement"
              }, {
                "argument" : {
                  "name" : "result",
                  "type" : "Identifier"
                },
                "type" : "ReturnStatement"
              } ],
              "type" : "BlockStatement"
            },
            "generator" : false,
            "type" : "FunctionExpression"
          },
          "kind" : "method",
          "computed" : false,
          "static" : false,
          "type" : "MethodDefinition"
        }, {
          "key" : {
            "name" : "editInternal",
            "type" : "Identifier"
          },
          "value" : {
            "params" : [ {
              "name" : "geometry",
              "type" : "Identifier"
            }, {
              "name" : "operation",
              "type" : "Identifier"
            } ],
            "body" : {
              "body" : [ {
                "test" : {
                  "operator" : "===",
                  "left" : {
                    "object" : {
                      "type" : "ThisExpression"
                    },
                    "property" : {
                      "name" : "_factory",
                      "type" : "Identifier"
                    },
                    "computed" : false,
                    "type" : "MemberExpression"
                  },
                  "right" : {
                    "value" : null,
                    "raw" : "null",
                    "type" : "Literal"
                  },
                  "type" : "BinaryExpression"
                },
                "consequent" : {
                  "expression" : {
                    "operator" : "=",
                    "left" : {
                      "object" : {
                        "type" : "ThisExpression"
                      },
                      "property" : {
                        "name" : "_factory",
                        "type" : "Identifier"
                      },
                      "computed" : false,
                      "type" : "MemberExpression"
                    },
                    "right" : {
                      "callee" : {
                        "object" : {
                          "name" : "geometry",
                          "type" : "Identifier"
                        },
                        "property" : {
                          "name" : "getFactory",
                          "type" : "Identifier"
                        },
                        "computed" : false,
                        "type" : "MemberExpression"
                      },
                      "arguments" : [ ],
                      "type" : "CallExpression"
                    },
                    "type" : "AssignmentExpression"
                  },
                  "type" : "ExpressionStatement"
                },
                "alternate" : null,
                "type" : "IfStatement"
              }, {
                "test" : {
                  "operator" : "instanceof",
                  "left" : {
                    "name" : "geometry",
                    "type" : "Identifier"
                  },
                  "right" : {
                    "name" : "GeometryCollection",
                    "type" : "Identifier"
                  },
                  "type" : "BinaryExpression"
                },
                "consequent" : {
                  "body" : [ {
                    "argument" : {
                      "callee" : {
                        "object" : {
                          "type" : "ThisExpression"
                        },
                        "property" : {
                          "name" : "editGeometryCollection",
                          "type" : "Identifier"
                        },
                        "computed" : false,
                        "type" : "MemberExpression"
                      },
                      "arguments" : [ {
                        "name" : "geometry",
                        "type" : "Identifier"
                      }, {
                        "name" : "operation",
                        "type" : "Identifier"
                      } ],
                      "type" : "CallExpression"
                    },
                    "type" : "ReturnStatement"
                  } ],
                  "type" : "BlockStatement"
                },
                "alternate" : null,
                "type" : "IfStatement"
              }, {
                "test" : {
                  "operator" : "instanceof",
                  "left" : {
                    "name" : "geometry",
                    "type" : "Identifier"
                  },
                  "right" : {
                    "name" : "Polygon",
                    "type" : "Identifier"
                  },
                  "type" : "BinaryExpression"
                },
                "consequent" : {
                  "body" : [ {
                    "argument" : {
                      "callee" : {
                        "object" : {
                          "type" : "ThisExpression"
                        },
                        "property" : {
                          "name" : "editPolygon",
                          "type" : "Identifier"
                        },
                        "computed" : false,
                        "type" : "MemberExpression"
                      },
                      "arguments" : [ {
                        "name" : "geometry",
                        "type" : "Identifier"
                      }, {
                        "name" : "operation",
                        "type" : "Identifier"
                      } ],
                      "type" : "CallExpression"
                    },
                    "type" : "ReturnStatement"
                  } ],
                  "type" : "BlockStatement"
                },
                "alternate" : null,
                "type" : "IfStatement"
              }, {
                "test" : {
                  "operator" : "instanceof",
                  "left" : {
                    "name" : "geometry",
                    "type" : "Identifier"
                  },
                  "right" : {
                    "name" : "Point",
                    "type" : "Identifier"
                  },
                  "type" : "BinaryExpression"
                },
                "consequent" : {
                  "body" : [ {
                    "argument" : {
                      "callee" : {
                        "object" : {
                          "name" : "operation",
                          "type" : "Identifier"
                        },
                        "property" : {
                          "name" : "edit",
                          "type" : "Identifier"
                        },
                        "computed" : false,
                        "type" : "MemberExpression"
                      },
                      "arguments" : [ {
                        "name" : "geometry",
                        "type" : "Identifier"
                      }, {
                        "object" : {
                          "type" : "ThisExpression"
                        },
                        "property" : {
                          "name" : "_factory",
                          "type" : "Identifier"
                        },
                        "computed" : false,
                        "type" : "MemberExpression"
                      } ],
                      "type" : "CallExpression"
                    },
                    "type" : "ReturnStatement"
                  } ],
                  "type" : "BlockStatement"
                },
                "alternate" : null,
                "type" : "IfStatement"
              }, {
                "test" : {
                  "operator" : "instanceof",
                  "left" : {
                    "name" : "geometry",
                    "type" : "Identifier"
                  },
                  "right" : {
                    "name" : "LineString",
                    "type" : "Identifier"
                  },
                  "type" : "BinaryExpression"
                },
                "consequent" : {
                  "body" : [ {
                    "argument" : {
                      "callee" : {
                        "object" : {
                          "name" : "operation",
                          "type" : "Identifier"
                        },
                        "property" : {
                          "name" : "edit",
                          "type" : "Identifier"
                        },
                        "computed" : false,
                        "type" : "MemberExpression"
                      },
                      "arguments" : [ {
                        "name" : "geometry",
                        "type" : "Identifier"
                      }, {
                        "object" : {
                          "type" : "ThisExpression"
                        },
                        "property" : {
                          "name" : "_factory",
                          "type" : "Identifier"
                        },
                        "computed" : false,
                        "type" : "MemberExpression"
                      } ],
                      "type" : "CallExpression"
                    },
                    "type" : "ReturnStatement"
                  } ],
                  "type" : "BlockStatement"
                },
                "alternate" : null,
                "type" : "IfStatement"
              }, {
                "expression" : {
                  "callee" : {
                    "object" : {
                      "name" : "Assert",
                      "type" : "Identifier"
                    },
                    "property" : {
                      "name" : "shouldNeverReachHere",
                      "type" : "Identifier"
                    },
                    "computed" : false,
                    "type" : "MemberExpression"
                  },
                  "arguments" : [ {
                    "operator" : "+",
                    "left" : {
                      "value" : "Unsupported Geometry type: ",
                      "raw" : "\"Unsupported Geometry type: \"",
                      "type" : "Literal"
                    },
                    "right" : {
                      "callee" : {
                        "object" : {
                          "name" : "geometry",
                          "type" : "Identifier"
                        },
                        "property" : {
                          "name" : "getGeometryType",
                          "type" : "Identifier"
                        },
                        "computed" : false,
                        "type" : "MemberExpression"
                      },
                      "arguments" : [ ],
                      "type" : "CallExpression"
                    },
                    "type" : "BinaryExpression"
                  } ],
                  "type" : "CallExpression"
                },
                "type" : "ExpressionStatement"
              }, {
                "argument" : {
                  "value" : null,
                  "raw" : "null",
                  "type" : "Literal"
                },
                "type" : "ReturnStatement"
              } ],
              "type" : "BlockStatement"
            },
            "generator" : false,
            "type" : "FunctionExpression"
          },
          "kind" : "method",
          "computed" : false,
          "static" : false,
          "type" : "MethodDefinition"
        }, {
          "key" : {
            "name" : "editGeometryCollection",
            "type" : "Identifier"
          },
          "value" : {
            "params" : [ {
              "name" : "collection",
              "type" : "Identifier"
            }, {
              "name" : "operation",
              "type" : "Identifier"
            } ],
            "body" : {
              "body" : [ {
                "declarations" : [ {
                  "id" : {
                    "name" : "collectionForType",
                    "type" : "Identifier"
                  },
                  "init" : {
                    "callee" : {
                      "object" : {
                        "name" : "operation",
                        "type" : "Identifier"
                      },
                      "property" : {
                        "name" : "edit",
                        "type" : "Identifier"
                      },
                      "computed" : false,
                      "type" : "MemberExpression"
                    },
                    "arguments" : [ {
                      "name" : "collection",
                      "type" : "Identifier"
                    }, {
                      "object" : {
                        "type" : "ThisExpression"
                      },
                      "property" : {
                        "name" : "_factory",
                        "type" : "Identifier"
                      },
                      "computed" : false,
                      "type" : "MemberExpression"
                    } ],
                    "type" : "CallExpression"
                  },
                  "type" : "VariableDeclarator"
                } ],
                "kind" : "let",
                "type" : "VariableDeclaration"
              }, {
                "declarations" : [ {
                  "id" : {
                    "name" : "geometries",
                    "type" : "Identifier"
                  },
                  "init" : {
                    "callee" : {
                      "name" : "ArrayList",
                      "type" : "Identifier"
                    },
                    "arguments" : [ ],
                    "type" : "NewExpression"
                  },
                  "type" : "VariableDeclarator"
                } ],
                "kind" : "let",
                "type" : "VariableDeclaration"
              }, {
                "init" : {
                  "declarations" : [ {
                    "id" : {
                      "name" : "i",
                      "type" : "Identifier"
                    },
                    "init" : {
                      "value" : "0",
                      "raw" : "0",
                      "type" : "Literal"
                    },
                    "type" : "VariableDeclarator"
                  } ],
                  "kind" : "let",
                  "type" : "VariableDeclaration"
                },
                "test" : {
                  "operator" : "<",
                  "left" : {
                    "name" : "i",
                    "type" : "Identifier"
                  },
                  "right" : {
                    "callee" : {
                      "object" : {
                        "name" : "collectionForType",
                        "type" : "Identifier"
                      },
                      "property" : {
                        "name" : "getNumGeometries",
                        "type" : "Identifier"
                      },
                      "computed" : false,
                      "type" : "MemberExpression"
                    },
                    "arguments" : [ ],
                    "type" : "CallExpression"
                  },
                  "type" : "BinaryExpression"
                },
                "update" : {
                  "operator" : "++",
                  "prefix" : false,
                  "argument" : {
                    "name" : "i",
                    "type" : "Identifier"
                  },
                  "type" : "UnaryExpression"
                },
                "body" : {
                  "body" : [ {
                    "declarations" : [ {
                      "id" : {
                        "name" : "geometry",
                        "type" : "Identifier"
                      },
                      "init" : {
                        "callee" : {
                          "object" : {
                            "type" : "ThisExpression"
                          },
                          "property" : {
                            "name" : "edit",
                            "type" : "Identifier"
                          },
                          "computed" : false,
                          "type" : "MemberExpression"
                        },
                        "arguments" : [ {
                          "callee" : {
                            "object" : {
                              "name" : "collectionForType",
                              "type" : "Identifier"
                            },
                            "property" : {
                              "name" : "getGeometryN",
                              "type" : "Identifier"
                            },
                            "computed" : false,
                            "type" : "MemberExpression"
                          },
                          "arguments" : [ {
                            "name" : "i",
                            "type" : "Identifier"
                          } ],
                          "type" : "CallExpression"
                        }, {
                          "name" : "operation",
                          "type" : "Identifier"
                        } ],
                        "type" : "CallExpression"
                      },
                      "type" : "VariableDeclarator"
                    } ],
                    "kind" : "let",
                    "type" : "VariableDeclaration"
                  }, {
                    "test" : {
                      "operator" : "||",
                      "left" : {
                        "operator" : "===",
                        "left" : {
                          "name" : "geometry",
                          "type" : "Identifier"
                        },
                        "right" : {
                          "value" : null,
                          "raw" : "null",
                          "type" : "Literal"
                        },
                        "type" : "BinaryExpression"
                      },
                      "right" : {
                        "callee" : {
                          "object" : {
                            "name" : "geometry",
                            "type" : "Identifier"
                          },
                          "property" : {
                            "name" : "isEmpty",
                            "type" : "Identifier"
                          },
                          "computed" : false,
                          "type" : "MemberExpression"
                        },
                        "arguments" : [ ],
                        "type" : "CallExpression"
                      },
                      "type" : "BinaryExpression"
                    },
                    "consequent" : {
                      "body" : [ {
                        "label" : null,
                        "type" : "ContinueStatement"
                      } ],
                      "type" : "BlockStatement"
                    },
                    "alternate" : null,
                    "type" : "IfStatement"
                  }, {
                    "expression" : {
                      "callee" : {
                        "object" : {
                          "name" : "geometries",
                          "type" : "Identifier"
                        },
                        "property" : {
                          "name" : "add",
                          "type" : "Identifier"
                        },
                        "computed" : false,
                        "type" : "MemberExpression"
                      },
                      "arguments" : [ {
                        "name" : "geometry",
                        "type" : "Identifier"
                      } ],
                      "type" : "CallExpression"
                    },
                    "type" : "ExpressionStatement"
                  } ],
                  "type" : "BlockStatement"
                },
                "type" : "ForStatement"
              }, {
                "test" : {
                  "operator" : "===",
                  "left" : {
                    "callee" : {
                      "object" : {
                        "name" : "collectionForType",
                        "type" : "Identifier"
                      },
                      "property" : {
                        "name" : "getGeometryType",
                        "type" : "Identifier"
                      },
                      "computed" : false,
                      "type" : "MemberExpression"
                    },
                    "arguments" : [ ],
                    "type" : "CallExpression"
                  },
                  "right" : {
                    "object" : {
                      "name" : "Geometry",
                      "type" : "Identifier"
                    },
                    "property" : {
                      "name" : "TYPENAME_MULTIPOINT",
                      "type" : "Identifier"
                    },
                    "computed" : false,
                    "type" : "MemberExpression"
                  },
                  "type" : "BinaryExpression"
                },
                "consequent" : {
                  "body" : [ {
                    "argument" : {
                      "callee" : {
                        "object" : {
                          "object" : {
                            "type" : "ThisExpression"
                          },
                          "property" : {
                            "name" : "_factory",
                            "type" : "Identifier"
                          },
                          "computed" : false,
                          "type" : "MemberExpression"
                        },
                        "property" : {
                          "name" : "createMultiPoint",
                          "type" : "Identifier"
                        },
                        "computed" : false,
                        "type" : "MemberExpression"
                      },
                      "arguments" : [ {
                        "callee" : {
                          "object" : {
                            "name" : "geometries",
                            "type" : "Identifier"
                          },
                          "property" : {
                            "name" : "toArray",
                            "type" : "Identifier"
                          },
                          "computed" : false,
                          "type" : "MemberExpression"
                        },
                        "arguments" : [ {
                          "elements" : [ ],
                          "type" : "ArrayExpression"
                        } ],
                        "type" : "CallExpression"
                      } ],
                      "type" : "CallExpression"
                    },
                    "type" : "ReturnStatement"
                  } ],
                  "type" : "BlockStatement"
                },
                "alternate" : null,
                "type" : "IfStatement"
              }, {
                "test" : {
                  "operator" : "===",
                  "left" : {
                    "callee" : {
                      "object" : {
                        "name" : "collectionForType",
                        "type" : "Identifier"
                      },
                      "property" : {
                        "name" : "getGeometryType",
                        "type" : "Identifier"
                      },
                      "computed" : false,
                      "type" : "MemberExpression"
                    },
                    "arguments" : [ ],
                    "type" : "CallExpression"
                  },
                  "right" : {
                    "object" : {
                      "name" : "Geometry",
                      "type" : "Identifier"
                    },
                    "property" : {
                      "name" : "TYPENAME_MULTILINESTRING",
                      "type" : "Identifier"
                    },
                    "computed" : false,
                    "type" : "MemberExpression"
                  },
                  "type" : "BinaryExpression"
                },
                "consequent" : {
                  "body" : [ {
                    "argument" : {
                      "callee" : {
                        "object" : {
                          "object" : {
                            "type" : "ThisExpression"
                          },
                          "property" : {
                            "name" : "_factory",
                            "type" : "Identifier"
                          },
                          "computed" : false,
                          "type" : "MemberExpression"
                        },
                        "property" : {
                          "name" : "createMultiLineString",
                          "type" : "Identifier"
                        },
                        "computed" : false,
                        "type" : "MemberExpression"
                      },
                      "arguments" : [ {
                        "callee" : {
                          "object" : {
                            "name" : "geometries",
                            "type" : "Identifier"
                          },
                          "property" : {
                            "name" : "toArray",
                            "type" : "Identifier"
                          },
                          "computed" : false,
                          "type" : "MemberExpression"
                        },
                        "arguments" : [ {
                          "elements" : [ ],
                          "type" : "ArrayExpression"
                        } ],
                        "type" : "CallExpression"
                      } ],
                      "type" : "CallExpression"
                    },
                    "type" : "ReturnStatement"
                  } ],
                  "type" : "BlockStatement"
                },
                "alternate" : null,
                "type" : "IfStatement"
              }, {
                "test" : {
                  "operator" : "===",
                  "left" : {
                    "callee" : {
                      "object" : {
                        "name" : "collectionForType",
                        "type" : "Identifier"
                      },
                      "property" : {
                        "name" : "getGeometryType",
                        "type" : "Identifier"
                      },
                      "computed" : false,
                      "type" : "MemberExpression"
                    },
                    "arguments" : [ ],
                    "type" : "CallExpression"
                  },
                  "right" : {
                    "object" : {
                      "name" : "Geometry",
                      "type" : "Identifier"
                    },
                    "property" : {
                      "name" : "TYPENAME_MULTIPOLYGON",
                      "type" : "Identifier"
                    },
                    "computed" : false,
                    "type" : "MemberExpression"
                  },
                  "type" : "BinaryExpression"
                },
                "consequent" : {
                  "body" : [ {
                    "argument" : {
                      "callee" : {
                        "object" : {
                          "object" : {
                            "type" : "ThisExpression"
                          },
                          "property" : {
                            "name" : "_factory",
                            "type" : "Identifier"
                          },
                          "computed" : false,
                          "type" : "MemberExpression"
                        },
                        "property" : {
                          "name" : "createMultiPolygon",
                          "type" : "Identifier"
                        },
                        "computed" : false,
                        "type" : "MemberExpression"
                      },
                      "arguments" : [ {
                        "callee" : {
                          "object" : {
                            "name" : "geometries",
                            "type" : "Identifier"
                          },
                          "property" : {
                            "name" : "toArray",
                            "type" : "Identifier"
                          },
                          "computed" : false,
                          "type" : "MemberExpression"
                        },
                        "arguments" : [ {
                          "elements" : [ ],
                          "type" : "ArrayExpression"
                        } ],
                        "type" : "CallExpression"
                      } ],
                      "type" : "CallExpression"
                    },
                    "type" : "ReturnStatement"
                  } ],
                  "type" : "BlockStatement"
                },
                "alternate" : null,
                "type" : "IfStatement"
              }, {
                "argument" : {
                  "callee" : {
                    "object" : {
                      "object" : {
                        "type" : "ThisExpression"
                      },
                      "property" : {
                        "name" : "_factory",
                        "type" : "Identifier"
                      },
                      "computed" : false,
                      "type" : "MemberExpression"
                    },
                    "property" : {
                      "name" : "createGeometryCollection",
                      "type" : "Identifier"
                    },
                    "computed" : false,
                    "type" : "MemberExpression"
                  },
                  "arguments" : [ {
                    "callee" : {
                      "object" : {
                        "name" : "geometries",
                        "type" : "Identifier"
                      },
                      "property" : {
                        "name" : "toArray",
                        "type" : "Identifier"
                      },
                      "computed" : false,
                      "type" : "MemberExpression"
                    },
                    "arguments" : [ {
                      "elements" : [ ],
                      "type" : "ArrayExpression"
                    } ],
                    "type" : "CallExpression"
                  } ],
                  "type" : "CallExpression"
                },
                "type" : "ReturnStatement"
              } ],
              "type" : "BlockStatement"
            },
            "generator" : false,
            "type" : "FunctionExpression"
          },
          "kind" : "method",
          "computed" : false,
          "static" : false,
          "type" : "MethodDefinition"
        }, {
          "key" : {
            "name" : "editPolygon",
            "type" : "Identifier"
          },
          "value" : {
            "params" : [ {
              "name" : "polygon",
              "type" : "Identifier"
            }, {
              "name" : "operation",
              "type" : "Identifier"
            } ],
            "body" : {
              "body" : [ {
                "declarations" : [ {
                  "id" : {
                    "name" : "newPolygon",
                    "type" : "Identifier"
                  },
                  "init" : {
                    "callee" : {
                      "object" : {
                        "name" : "operation",
                        "type" : "Identifier"
                      },
                      "property" : {
                        "name" : "edit",
                        "type" : "Identifier"
                      },
                      "computed" : false,
                      "type" : "MemberExpression"
                    },
                    "arguments" : [ {
                      "name" : "polygon",
                      "type" : "Identifier"
                    }, {
                      "object" : {
                        "type" : "ThisExpression"
                      },
                      "property" : {
                        "name" : "_factory",
                        "type" : "Identifier"
                      },
                      "computed" : false,
                      "type" : "MemberExpression"
                    } ],
                    "type" : "CallExpression"
                  },
                  "type" : "VariableDeclarator"
                } ],
                "kind" : "let",
                "type" : "VariableDeclaration"
              }, {
                "test" : {
                  "operator" : "===",
                  "left" : {
                    "name" : "newPolygon",
                    "type" : "Identifier"
                  },
                  "right" : {
                    "value" : null,
                    "raw" : "null",
                    "type" : "Literal"
                  },
                  "type" : "BinaryExpression"
                },
                "consequent" : {
                  "expression" : {
                    "operator" : "=",
                    "left" : {
                      "name" : "newPolygon",
                      "type" : "Identifier"
                    },
                    "right" : {
                      "callee" : {
                        "object" : {
                          "object" : {
                            "type" : "ThisExpression"
                          },
                          "property" : {
                            "name" : "_factory",
                            "type" : "Identifier"
                          },
                          "computed" : false,
                          "type" : "MemberExpression"
                        },
                        "property" : {
                          "name" : "createPolygon",
                          "type" : "Identifier"
                        },
                        "computed" : false,
                        "type" : "MemberExpression"
                      },
                      "arguments" : [ ],
                      "type" : "CallExpression"
                    },
                    "type" : "AssignmentExpression"
                  },
                  "type" : "ExpressionStatement"
                },
                "alternate" : null,
                "type" : "IfStatement"
              }, {
                "test" : {
                  "callee" : {
                    "object" : {
                      "name" : "newPolygon",
                      "type" : "Identifier"
                    },
                    "property" : {
                      "name" : "isEmpty",
                      "type" : "Identifier"
                    },
                    "computed" : false,
                    "type" : "MemberExpression"
                  },
                  "arguments" : [ ],
                  "type" : "CallExpression"
                },
                "consequent" : {
                  "body" : [ {
                    "argument" : {
                      "name" : "newPolygon",
                      "type" : "Identifier"
                    },
                    "type" : "ReturnStatement"
                  } ],
                  "type" : "BlockStatement"
                },
                "alternate" : null,
                "type" : "IfStatement"
              }, {
                "declarations" : [ {
                  "id" : {
                    "name" : "shell",
                    "type" : "Identifier"
                  },
                  "init" : {
                    "callee" : {
                      "object" : {
                        "type" : "ThisExpression"
                      },
                      "property" : {
                        "name" : "edit",
                        "type" : "Identifier"
                      },
                      "computed" : false,
                      "type" : "MemberExpression"
                    },
                    "arguments" : [ {
                      "callee" : {
                        "object" : {
                          "name" : "newPolygon",
                          "type" : "Identifier"
                        },
                        "property" : {
                          "name" : "getExteriorRing",
                          "type" : "Identifier"
                        },
                        "computed" : false,
                        "type" : "MemberExpression"
                      },
                      "arguments" : [ ],
                      "type" : "CallExpression"
                    }, {
                      "name" : "operation",
                      "type" : "Identifier"
                    } ],
                    "type" : "CallExpression"
                  },
                  "type" : "VariableDeclarator"
                } ],
                "kind" : "let",
                "type" : "VariableDeclaration"
              }, {
                "test" : {
                  "operator" : "||",
                  "left" : {
                    "operator" : "===",
                    "left" : {
                      "name" : "shell",
                      "type" : "Identifier"
                    },
                    "right" : {
                      "value" : null,
                      "raw" : "null",
                      "type" : "Literal"
                    },
                    "type" : "BinaryExpression"
                  },
                  "right" : {
                    "callee" : {
                      "object" : {
                        "name" : "shell",
                        "type" : "Identifier"
                      },
                      "property" : {
                        "name" : "isEmpty",
                        "type" : "Identifier"
                      },
                      "computed" : false,
                      "type" : "MemberExpression"
                    },
                    "arguments" : [ ],
                    "type" : "CallExpression"
                  },
                  "type" : "BinaryExpression"
                },
                "consequent" : {
                  "body" : [ {
                    "argument" : {
                      "callee" : {
                        "object" : {
                          "object" : {
                            "type" : "ThisExpression"
                          },
                          "property" : {
                            "name" : "_factory",
                            "type" : "Identifier"
                          },
                          "computed" : false,
                          "type" : "MemberExpression"
                        },
                        "property" : {
                          "name" : "createPolygon",
                          "type" : "Identifier"
                        },
                        "computed" : false,
                        "type" : "MemberExpression"
                      },
                      "arguments" : [ ],
                      "type" : "CallExpression"
                    },
                    "type" : "ReturnStatement"
                  } ],
                  "type" : "BlockStatement"
                },
                "alternate" : null,
                "type" : "IfStatement"
              }, {
                "declarations" : [ {
                  "id" : {
                    "name" : "holes",
                    "type" : "Identifier"
                  },
                  "init" : {
                    "callee" : {
                      "name" : "ArrayList",
                      "type" : "Identifier"
                    },
                    "arguments" : [ ],
                    "type" : "NewExpression"
                  },
                  "type" : "VariableDeclarator"
                } ],
                "kind" : "let",
                "type" : "VariableDeclaration"
              }, {
                "init" : {
                  "declarations" : [ {
                    "id" : {
                      "name" : "i",
                      "type" : "Identifier"
                    },
                    "init" : {
                      "value" : "0",
                      "raw" : "0",
                      "type" : "Literal"
                    },
                    "type" : "VariableDeclarator"
                  } ],
                  "kind" : "let",
                  "type" : "VariableDeclaration"
                },
                "test" : {
                  "operator" : "<",
                  "left" : {
                    "name" : "i",
                    "type" : "Identifier"
                  },
                  "right" : {
                    "callee" : {
                      "object" : {
                        "name" : "newPolygon",
                        "type" : "Identifier"
                      },
                      "property" : {
                        "name" : "getNumInteriorRing",
                        "type" : "Identifier"
                      },
                      "computed" : false,
                      "type" : "MemberExpression"
                    },
                    "arguments" : [ ],
                    "type" : "CallExpression"
                  },
                  "type" : "BinaryExpression"
                },
                "update" : {
                  "operator" : "++",
                  "prefix" : false,
                  "argument" : {
                    "name" : "i",
                    "type" : "Identifier"
                  },
                  "type" : "UnaryExpression"
                },
                "body" : {
                  "body" : [ {
                    "declarations" : [ {
                      "id" : {
                        "name" : "hole",
                        "type" : "Identifier"
                      },
                      "init" : {
                        "callee" : {
                          "object" : {
                            "type" : "ThisExpression"
                          },
                          "property" : {
                            "name" : "edit",
                            "type" : "Identifier"
                          },
                          "computed" : false,
                          "type" : "MemberExpression"
                        },
                        "arguments" : [ {
                          "callee" : {
                            "object" : {
                              "name" : "newPolygon",
                              "type" : "Identifier"
                            },
                            "property" : {
                              "name" : "getInteriorRingN",
                              "type" : "Identifier"
                            },
                            "computed" : false,
                            "type" : "MemberExpression"
                          },
                          "arguments" : [ {
                            "name" : "i",
                            "type" : "Identifier"
                          } ],
                          "type" : "CallExpression"
                        }, {
                          "name" : "operation",
                          "type" : "Identifier"
                        } ],
                        "type" : "CallExpression"
                      },
                      "type" : "VariableDeclarator"
                    } ],
                    "kind" : "let",
                    "type" : "VariableDeclaration"
                  }, {
                    "test" : {
                      "operator" : "||",
                      "left" : {
                        "operator" : "===",
                        "left" : {
                          "name" : "hole",
                          "type" : "Identifier"
                        },
                        "right" : {
                          "value" : null,
                          "raw" : "null",
                          "type" : "Literal"
                        },
                        "type" : "BinaryExpression"
                      },
                      "right" : {
                        "callee" : {
                          "object" : {
                            "name" : "hole",
                            "type" : "Identifier"
                          },
                          "property" : {
                            "name" : "isEmpty",
                            "type" : "Identifier"
                          },
                          "computed" : false,
                          "type" : "MemberExpression"
                        },
                        "arguments" : [ ],
                        "type" : "CallExpression"
                      },
                      "type" : "BinaryExpression"
                    },
                    "consequent" : {
                      "body" : [ {
                        "label" : null,
                        "type" : "ContinueStatement"
                      } ],
                      "type" : "BlockStatement"
                    },
                    "alternate" : null,
                    "type" : "IfStatement"
                  }, {
                    "expression" : {
                      "callee" : {
                        "object" : {
                          "name" : "holes",
                          "type" : "Identifier"
                        },
                        "property" : {
                          "name" : "add",
                          "type" : "Identifier"
                        },
                        "computed" : false,
                        "type" : "MemberExpression"
                      },
                      "arguments" : [ {
                        "name" : "hole",
                        "type" : "Identifier"
                      } ],
                      "type" : "CallExpression"
                    },
                    "type" : "ExpressionStatement"
                  } ],
                  "type" : "BlockStatement"
                },
                "type" : "ForStatement"
              }, {
                "argument" : {
                  "callee" : {
                    "object" : {
                      "object" : {
                        "type" : "ThisExpression"
                      },
                      "property" : {
                        "name" : "_factory",
                        "type" : "Identifier"
                      },
                      "computed" : false,
                      "type" : "MemberExpression"
                    },
                    "property" : {
                      "name" : "createPolygon",
                      "type" : "Identifier"
                    },
                    "computed" : false,
                    "type" : "MemberExpression"
                  },
                  "arguments" : [ {
                    "name" : "shell",
                    "type" : "Identifier"
                  }, {
                    "callee" : {
                      "object" : {
                        "name" : "holes",
                        "type" : "Identifier"
                      },
                      "property" : {
                        "name" : "toArray",
                        "type" : "Identifier"
                      },
                      "computed" : false,
                      "type" : "MemberExpression"
                    },
                    "arguments" : [ {
                      "elements" : [ ],
                      "type" : "ArrayExpression"
                    } ],
                    "type" : "CallExpression"
                  } ],
                  "type" : "CallExpression"
                },
                "type" : "ReturnStatement"
              } ],
              "type" : "BlockStatement"
            },
            "generator" : false,
            "type" : "FunctionExpression"
          },
          "kind" : "method",
          "computed" : false,
          "static" : false,
          "type" : "MethodDefinition"
        } ],
        "type" : "ClassBody"
      },
      "superClass" : null,
      "type" : "ClassDeclaration"
    },
    "type" : "ExportDefaultDeclaration"
  }, {
    "id" : {
      "name" : "GeometryEditorOperation",
      "type" : "Identifier"
    },
    "params" : [ ],
    "body" : {
      "body" : [ ],
      "type" : "BlockStatement"
    },
    "generator" : false,
    "type" : "FunctionDeclaration"
  }, {
    "expression" : {
      "operator" : "=",
      "left" : {
        "object" : {
          "name" : "GeometryEditor",
          "type" : "Identifier"
        },
        "property" : {
          "name" : "GeometryEditorOperation",
          "type" : "Identifier"
        },
        "computed" : false,
        "type" : "MemberExpression"
      },
      "right" : {
        "name" : "GeometryEditorOperation",
        "type" : "Identifier"
      },
      "type" : "AssignmentExpression"
    },
    "type" : "ExpressionStatement"
  }, {
    "id" : {
      "name" : "NoOpGeometryOperation",
      "type" : "Identifier"
    },
    "body" : {
      "body" : [ {
        "key" : {
          "name" : "edit",
          "type" : "Identifier"
        },
        "value" : {
          "params" : [ {
            "name" : "geometry",
            "type" : "Identifier"
          }, {
            "name" : "factory",
            "type" : "Identifier"
          } ],
          "body" : {
            "body" : [ {
              "argument" : {
                "name" : "geometry",
                "type" : "Identifier"
              },
              "type" : "ReturnStatement"
            } ],
            "type" : "BlockStatement"
          },
          "generator" : false,
          "type" : "FunctionExpression"
        },
        "kind" : "method",
        "computed" : false,
        "static" : false,
        "type" : "MethodDefinition"
      }, {
        "key" : {
          "name" : "interfaces_",
          "type" : "Identifier"
        },
        "value" : {
          "params" : [ ],
          "body" : {
            "body" : [ {
              "argument" : {
                "elements" : [ {
                  "name" : "GeometryEditorOperation",
                  "type" : "Identifier"
                } ],
                "type" : "ArrayExpression"
              },
              "type" : "ReturnStatement"
            } ],
            "type" : "BlockStatement"
          },
          "generator" : false,
          "type" : "FunctionExpression"
        },
        "kind" : "get",
        "computed" : false,
        "static" : false,
        "type" : "MethodDefinition"
      } ],
      "type" : "ClassBody"
    },
    "superClass" : null,
    "type" : "ClassDeclaration"
  }, {
    "id" : {
      "name" : "CoordinateOperation",
      "type" : "Identifier"
    },
    "body" : {
      "body" : [ {
        "key" : {
          "name" : "edit",
          "type" : "Identifier"
        },
        "value" : {
          "params" : [ {
            "name" : "geometry",
            "type" : "Identifier"
          }, {
            "name" : "factory",
            "type" : "Identifier"
          } ],
          "body" : {
            "body" : [ {
              "declarations" : [ {
                "id" : {
                  "name" : "coordinates",
                  "type" : "Identifier"
                },
                "init" : {
                  "callee" : {
                    "object" : {
                      "type" : "ThisExpression"
                    },
                    "property" : {
                      "name" : "edit",
                      "type" : "Identifier"
                    },
                    "computed" : false,
                    "type" : "MemberExpression"
                  },
                  "arguments" : [ {
                    "callee" : {
                      "object" : {
                        "name" : "geometry",
                        "type" : "Identifier"
                      },
                      "property" : {
                        "name" : "getCoordinates",
                        "type" : "Identifier"
                      },
                      "computed" : false,
                      "type" : "MemberExpression"
                    },
                    "arguments" : [ ],
                    "type" : "CallExpression"
                  }, {
                    "name" : "geometry",
                    "type" : "Identifier"
                  } ],
                  "type" : "CallExpression"
                },
                "type" : "VariableDeclarator"
              } ],
              "kind" : "let",
              "type" : "VariableDeclaration"
            }, {
              "test" : {
                "operator" : "instanceof",
                "left" : {
                  "name" : "geometry",
                  "type" : "Identifier"
                },
                "right" : {
                  "name" : "LinearRing",
                  "type" : "Identifier"
                },
                "type" : "BinaryExpression"
              },
              "consequent" : {
                "body" : [ {
                  "test" : {
                    "operator" : "===",
                    "left" : {
                      "name" : "coordinates",
                      "type" : "Identifier"
                    },
                    "right" : {
                      "value" : null,
                      "raw" : "null",
                      "type" : "Literal"
                    },
                    "type" : "BinaryExpression"
                  },
                  "consequent" : {
                    "argument" : {
                      "callee" : {
                        "object" : {
                          "name" : "factory",
                          "type" : "Identifier"
                        },
                        "property" : {
                          "name" : "createLinearRing",
                          "type" : "Identifier"
                        },
                        "computed" : false,
                        "type" : "MemberExpression"
                      },
                      "arguments" : [ ],
                      "type" : "CallExpression"
                    },
                    "type" : "ReturnStatement"
                  },
                  "alternate" : {
                    "argument" : {
                      "callee" : {
                        "object" : {
                          "name" : "factory",
                          "type" : "Identifier"
                        },
                        "property" : {
                          "name" : "createLinearRing",
                          "type" : "Identifier"
                        },
                        "computed" : false,
                        "type" : "MemberExpression"
                      },
                      "arguments" : [ {
                        "name" : "coordinates",
                        "type" : "Identifier"
                      } ],
                      "type" : "CallExpression"
                    },
                    "type" : "ReturnStatement"
                  },
                  "type" : "IfStatement"
                } ],
                "type" : "BlockStatement"
              },
              "alternate" : null,
              "type" : "IfStatement"
            }, {
              "test" : {
                "operator" : "instanceof",
                "left" : {
                  "name" : "geometry",
                  "type" : "Identifier"
                },
                "right" : {
                  "name" : "LineString",
                  "type" : "Identifier"
                },
                "type" : "BinaryExpression"
              },
              "consequent" : {
                "body" : [ {
                  "test" : {
                    "operator" : "===",
                    "left" : {
                      "name" : "coordinates",
                      "type" : "Identifier"
                    },
                    "right" : {
                      "value" : null,
                      "raw" : "null",
                      "type" : "Literal"
                    },
                    "type" : "BinaryExpression"
                  },
                  "consequent" : {
                    "argument" : {
                      "callee" : {
                        "object" : {
                          "name" : "factory",
                          "type" : "Identifier"
                        },
                        "property" : {
                          "name" : "createLineString",
                          "type" : "Identifier"
                        },
                        "computed" : false,
                        "type" : "MemberExpression"
                      },
                      "arguments" : [ ],
                      "type" : "CallExpression"
                    },
                    "type" : "ReturnStatement"
                  },
                  "alternate" : {
                    "argument" : {
                      "callee" : {
                        "object" : {
                          "name" : "factory",
                          "type" : "Identifier"
                        },
                        "property" : {
                          "name" : "createLineString",
                          "type" : "Identifier"
                        },
                        "computed" : false,
                        "type" : "MemberExpression"
                      },
                      "arguments" : [ {
                        "name" : "coordinates",
                        "type" : "Identifier"
                      } ],
                      "type" : "CallExpression"
                    },
                    "type" : "ReturnStatement"
                  },
                  "type" : "IfStatement"
                } ],
                "type" : "BlockStatement"
              },
              "alternate" : null,
              "type" : "IfStatement"
            }, {
              "test" : {
                "operator" : "instanceof",
                "left" : {
                  "name" : "geometry",
                  "type" : "Identifier"
                },
                "right" : {
                  "name" : "Point",
                  "type" : "Identifier"
                },
                "type" : "BinaryExpression"
              },
              "consequent" : {
                "body" : [ {
                  "test" : {
                    "operator" : "||",
                    "left" : {
                      "operator" : "===",
                      "left" : {
                        "name" : "coordinates",
                        "type" : "Identifier"
                      },
                      "right" : {
                        "value" : null,
                        "raw" : "null",
                        "type" : "Literal"
                      },
                      "type" : "BinaryExpression"
                    },
                    "right" : {
                      "operator" : "===",
                      "left" : {
                        "object" : {
                          "name" : "coordinates",
                          "type" : "Identifier"
                        },
                        "property" : {
                          "name" : "length",
                          "type" : "Identifier"
                        },
                        "computed" : false,
                        "type" : "MemberExpression"
                      },
                      "right" : {
                        "value" : "0",
                        "raw" : "0",
                        "type" : "Literal"
                      },
                      "type" : "BinaryExpression"
                    },
                    "type" : "BinaryExpression"
                  },
                  "consequent" : {
                    "argument" : {
                      "callee" : {
                        "object" : {
                          "name" : "factory",
                          "type" : "Identifier"
                        },
                        "property" : {
                          "name" : "createPoint",
                          "type" : "Identifier"
                        },
                        "computed" : false,
                        "type" : "MemberExpression"
                      },
                      "arguments" : [ ],
                      "type" : "CallExpression"
                    },
                    "type" : "ReturnStatement"
                  },
                  "alternate" : {
                    "argument" : {
                      "callee" : {
                        "object" : {
                          "name" : "factory",
                          "type" : "Identifier"
                        },
                        "property" : {
                          "name" : "createPoint",
                          "type" : "Identifier"
                        },
                        "computed" : false,
                        "type" : "MemberExpression"
                      },
                      "arguments" : [ {
                        "object" : {
                          "name" : "coordinates",
                          "type" : "Identifier"
                        },
                        "property" : {
                          "value" : "0",
                          "raw" : "0",
                          "type" : "Literal"
                        },
                        "computed" : true,
                        "type" : "MemberExpression"
                      } ],
                      "type" : "CallExpression"
                    },
                    "type" : "ReturnStatement"
                  },
                  "type" : "IfStatement"
                } ],
                "type" : "BlockStatement"
              },
              "alternate" : null,
              "type" : "IfStatement"
            }, {
              "argument" : {
                "name" : "geometry",
                "type" : "Identifier"
              },
              "type" : "ReturnStatement"
            } ],
            "type" : "BlockStatement"
          },
          "generator" : false,
          "type" : "FunctionExpression"
        },
        "kind" : "method",
        "computed" : false,
        "static" : false,
        "type" : "MethodDefinition"
      }, {
        "key" : {
          "name" : "interfaces_",
          "type" : "Identifier"
        },
        "value" : {
          "params" : [ ],
          "body" : {
            "body" : [ {
              "argument" : {
                "elements" : [ {
                  "name" : "GeometryEditorOperation",
                  "type" : "Identifier"
                } ],
                "type" : "ArrayExpression"
              },
              "type" : "ReturnStatement"
            } ],
            "type" : "BlockStatement"
          },
          "generator" : false,
          "type" : "FunctionExpression"
        },
        "kind" : "get",
        "computed" : false,
        "static" : false,
        "type" : "MethodDefinition"
      } ],
      "type" : "ClassBody"
    },
    "superClass" : null,
    "type" : "ClassDeclaration"
  }, {
    "id" : {
      "name" : "CoordinateSequenceOperation",
      "type" : "Identifier"
    },
    "body" : {
      "body" : [ {
        "key" : {
          "name" : "edit",
          "type" : "Identifier"
        },
        "value" : {
          "params" : [ {
            "name" : "geometry",
            "type" : "Identifier"
          }, {
            "name" : "factory",
            "type" : "Identifier"
          } ],
          "body" : {
            "body" : [ {
              "test" : {
                "operator" : "instanceof",
                "left" : {
                  "name" : "geometry",
                  "type" : "Identifier"
                },
                "right" : {
                  "name" : "LinearRing",
                  "type" : "Identifier"
                },
                "type" : "BinaryExpression"
              },
              "consequent" : {
                "body" : [ {
                  "argument" : {
                    "callee" : {
                      "object" : {
                        "name" : "factory",
                        "type" : "Identifier"
                      },
                      "property" : {
                        "name" : "createLinearRing",
                        "type" : "Identifier"
                      },
                      "computed" : false,
                      "type" : "MemberExpression"
                    },
                    "arguments" : [ {
                      "callee" : {
                        "object" : {
                          "type" : "ThisExpression"
                        },
                        "property" : {
                          "name" : "edit",
                          "type" : "Identifier"
                        },
                        "computed" : false,
                        "type" : "MemberExpression"
                      },
                      "arguments" : [ {
                        "callee" : {
                          "object" : {
                            "name" : "geometry",
                            "type" : "Identifier"
                          },
                          "property" : {
                            "name" : "getCoordinateSequence",
                            "type" : "Identifier"
                          },
                          "computed" : false,
                          "type" : "MemberExpression"
                        },
                        "arguments" : [ ],
                        "type" : "CallExpression"
                      }, {
                        "name" : "geometry",
                        "type" : "Identifier"
                      } ],
                      "type" : "CallExpression"
                    } ],
                    "type" : "CallExpression"
                  },
                  "type" : "ReturnStatement"
                } ],
                "type" : "BlockStatement"
              },
              "alternate" : null,
              "type" : "IfStatement"
            }, {
              "test" : {
                "operator" : "instanceof",
                "left" : {
                  "name" : "geometry",
                  "type" : "Identifier"
                },
                "right" : {
                  "name" : "LineString",
                  "type" : "Identifier"
                },
                "type" : "BinaryExpression"
              },
              "consequent" : {
                "body" : [ {
                  "argument" : {
                    "callee" : {
                      "object" : {
                        "name" : "factory",
                        "type" : "Identifier"
                      },
                      "property" : {
                        "name" : "createLineString",
                        "type" : "Identifier"
                      },
                      "computed" : false,
                      "type" : "MemberExpression"
                    },
                    "arguments" : [ {
                      "callee" : {
                        "object" : {
                          "type" : "ThisExpression"
                        },
                        "property" : {
                          "name" : "edit",
                          "type" : "Identifier"
                        },
                        "computed" : false,
                        "type" : "MemberExpression"
                      },
                      "arguments" : [ {
                        "callee" : {
                          "object" : {
                            "name" : "geometry",
                            "type" : "Identifier"
                          },
                          "property" : {
                            "name" : "getCoordinateSequence",
                            "type" : "Identifier"
                          },
                          "computed" : false,
                          "type" : "MemberExpression"
                        },
                        "arguments" : [ ],
                        "type" : "CallExpression"
                      }, {
                        "name" : "geometry",
                        "type" : "Identifier"
                      } ],
                      "type" : "CallExpression"
                    } ],
                    "type" : "CallExpression"
                  },
                  "type" : "ReturnStatement"
                } ],
                "type" : "BlockStatement"
              },
              "alternate" : null,
              "type" : "IfStatement"
            }, {
              "test" : {
                "operator" : "instanceof",
                "left" : {
                  "name" : "geometry",
                  "type" : "Identifier"
                },
                "right" : {
                  "name" : "Point",
                  "type" : "Identifier"
                },
                "type" : "BinaryExpression"
              },
              "consequent" : {
                "body" : [ {
                  "argument" : {
                    "callee" : {
                      "object" : {
                        "name" : "factory",
                        "type" : "Identifier"
                      },
                      "property" : {
                        "name" : "createPoint",
                        "type" : "Identifier"
                      },
                      "computed" : false,
                      "type" : "MemberExpression"
                    },
                    "arguments" : [ {
                      "callee" : {
                        "object" : {
                          "type" : "ThisExpression"
                        },
                        "property" : {
                          "name" : "edit",
                          "type" : "Identifier"
                        },
                        "computed" : false,
                        "type" : "MemberExpression"
                      },
                      "arguments" : [ {
                        "callee" : {
                          "object" : {
                            "name" : "geometry",
                            "type" : "Identifier"
                          },
                          "property" : {
                            "name" : "getCoordinateSequence",
                            "type" : "Identifier"
                          },
                          "computed" : false,
                          "type" : "MemberExpression"
                        },
                        "arguments" : [ ],
                        "type" : "CallExpression"
                      }, {
                        "name" : "geometry",
                        "type" : "Identifier"
                      } ],
                      "type" : "CallExpression"
                    } ],
                    "type" : "CallExpression"
                  },
                  "type" : "ReturnStatement"
                } ],
                "type" : "BlockStatement"
              },
              "alternate" : null,
              "type" : "IfStatement"
            }, {
              "argument" : {
                "name" : "geometry",
                "type" : "Identifier"
              },
              "type" : "ReturnStatement"
            } ],
            "type" : "BlockStatement"
          },
          "generator" : false,
          "type" : "FunctionExpression"
        },
        "kind" : "method",
        "computed" : false,
        "static" : false,
        "type" : "MethodDefinition"
      }, {
        "key" : {
          "name" : "interfaces_",
          "type" : "Identifier"
        },
        "value" : {
          "params" : [ ],
          "body" : {
            "body" : [ {
              "argument" : {
                "elements" : [ {
                  "name" : "GeometryEditorOperation",
                  "type" : "Identifier"
                } ],
                "type" : "ArrayExpression"
              },
              "type" : "ReturnStatement"
            } ],
            "type" : "BlockStatement"
          },
          "generator" : false,
          "type" : "FunctionExpression"
        },
        "kind" : "get",
        "computed" : false,
        "static" : false,
        "type" : "MethodDefinition"
      } ],
      "type" : "ClassBody"
    },
    "superClass" : null,
    "type" : "ClassDeclaration"
  }, {
    "expression" : {
      "operator" : "=",
      "left" : {
        "object" : {
          "name" : "GeometryEditor",
          "type" : "Identifier"
        },
        "property" : {
          "name" : "NoOpGeometryOperation",
          "type" : "Identifier"
        },
        "computed" : false,
        "type" : "MemberExpression"
      },
      "right" : {
        "name" : "NoOpGeometryOperation",
        "type" : "Identifier"
      },
      "type" : "AssignmentExpression"
    },
    "type" : "ExpressionStatement"
  }, {
    "expression" : {
      "operator" : "=",
      "left" : {
        "object" : {
          "name" : "GeometryEditor",
          "type" : "Identifier"
        },
        "property" : {
          "name" : "CoordinateOperation",
          "type" : "Identifier"
        },
        "computed" : false,
        "type" : "MemberExpression"
      },
      "right" : {
        "name" : "CoordinateOperation",
        "type" : "Identifier"
      },
      "type" : "AssignmentExpression"
    },
    "type" : "ExpressionStatement"
  }, {
    "expression" : {
      "operator" : "=",
      "left" : {
        "object" : {
          "name" : "GeometryEditor",
          "type" : "Identifier"
        },
        "property" : {
          "name" : "CoordinateSequenceOperation",
          "type" : "Identifier"
        },
        "computed" : false,
        "type" : "MemberExpression"
      },
      "right" : {
        "name" : "CoordinateSequenceOperation",
        "type" : "Identifier"
      },
      "type" : "AssignmentExpression"
    },
    "type" : "ExpressionStatement"
  } ],
  "type" : "Program"
}